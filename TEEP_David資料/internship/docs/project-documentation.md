...
- **[Near-RT RIC Platform](nearrt-ric-installation-link) (10.10.4.10)**: The Near-Real-Time RAN Intelligent Controller provides sub-second (10 ms – 1 s) control loop capabilities for RAN optimization. Implemented using FlexRIC v1.0.0, it hosts xApps that receive real-time KPIs through the E2 interface [6] and execute control actions on CU/DU nodes. The platform provides an E2 Termination endpoint (Port 36421) that manages E2AP associations with RAN nodes, and an A1 interface [5] endpoint that receives high-level policies from the Non-RT RIC. For our energy saving use case, the Near-RT RIC hosts the ES xApp (energy saving decision engine) and the HO xApp (handover execution engine).

- **[Energy Saving xApp](es-xapp-installation-link) (Port 8080)**: The Energy Saving xApp implements the real-time energy optimization logic. It subscribes to E2 KPM (Key Performance Measurement) indications from the DU to monitor per-cell PRB utilization, active UE count, and throughput metrics at 100 ms intervals. When traffic falls below configurable thresholds (e.g., PRB utilization < 30% for > 5 minutes), it triggers the handover procedure via the HO xApp, followed by cell shutdown through E2 RIC Control messages. Conversely, when neighbor cell load exceeds 80%, it initiates cell reactivation.

- **[Handover xApp](ho-xapp-installation-link) (Port 8080)**: The Handover xApp manages UE mobility during cell shutdown/activation procedures. Upon receiving a handover trigger from the ES xApp, it sends E2 RIC Control Request messages to the CU to initiate X2/Xn-based inter-cell handover. It monitors handover completion status and reports back to the ES xApp, ensuring all active UEs are safely moved before cell deactivation proceeds.

#### [Central Unit - OAI [2024.w40]](oai-cu-installation-link)

- **[CU-CP (Control Plane)](cu-cp-installation-link)**: The Central Unit Control Plane handles RRC (Radio Resource Control) signaling, NAS (Non-Access Stratum) message forwarding, and UE context management. Implemented using OAI 2024.w40, it connects to the 5G Core (Open5GS) via the N2 interface (SCTP/NGAP) for AMF registration and to the DU via the F1-C interface for control-plane message relay. For energy saving, the CU-CP processes handover requests initiated by the Near-RT RIC via E2 and executes RRC Reconfiguration procedures to move UEs between cells.

- **[CU-UP (User Plane)](cu-up-installation-link)**: The Central Unit User Plane handles GTP-U tunneling for user data between the DU (F1-U interface) and the 5G Core (N3 interface). It manages SDAP (Service Data Adaptation Protocol) and PDCP (Packet Data Convergence Protocol) layers, performing header compression, ciphering, and integrity protection. The CU-UP's processing load scales linearly with active user throughput, contributing to the dynamic power component measured by our Scaphandre/RAPL methodology.

#### [Distributed Unit - OAI [2024.w40]](oai-du-installation-link)

- **[Distributed Unit](du-installation-link)**: The DU implements the RLC (Radio Link Control), MAC (Medium Access Control), and High-PHY layers of the NR protocol stack. It is the **key component for energy saving** because it hosts the MAC scheduler — the decision point that controls PRB allocation and, consequently, O-RU Power Amplifier duty cycle. The DU connects to the CU via F1 interface (F1-C for control, F1-U for user plane) and to the RU via the Open Fronthaul (eCPRI/O-FH) interface. It exposes E2 and O1 agents for RIC control and SMO management respectively. Our scheduler analysis (`docs/StudyNotes/2026-02-24_OAI-srsRAN-MAC-Scheduler-PRB-Scheduling.md`) identifies the DU's `gNB_scheduler_dlsch.c` (OAI) and `intra_slice_scheduler.cpp` (srsRAN) as the intervention points for FDM→TDM scheduling modification.

#### [Radio Unit - USRP B210](usrp-installation-link)

- **[Radio Unit / SDR](ru-installation-link)**: The Radio Unit implements the Low-PHY layer (FFT/iFFT, beamforming, digital-to-analog conversion) and the RF front-end (Power Amplifier, filters, antenna interface). In our lab setup, a USRP B210 SDR serves as the RU, connected to the DU via USB 3.0. For production O-RAN deployments, dedicated O-RUs (e.g., Foxconn RPQN, Benetel) connect via eCPRI fronthaul and support sleep modes SM0–SM3. The RU is the **dominant power consumer** in the RAN: our gap analysis (`docs/StudyNotes/2026-01-26_Day-9_Gap-Analysis-Proxy-vs-ORU.md`) showed that even at full CPU load (59W), the compute proxy is ~140W below the minimum idle power of a Macro O-RU (197W), confirming that the PA/RF/cooling components within the RU account for the majority of base station power consumption.

#### [User Equipment](ue-installation-link)

- **[UE Module — Quectel RM500Q-GL](ue-installation-link)**: The User Equipment connects to the gNB over the Uu (air) interface for 5G NR Standalone (SA) operation. In our lab, a Quectel RM500Q-GL 5G modem is used for over-the-air testing. For virtualized experiments (ZMQ mode), we use `srsRAN_4G`'s `srsue` as a software UE emulator, which supports 5G SA attachment, PDU Session establishment, and data transfer through the simulated radio link. The UE generates controlled traffic loads via `iperf3` to drive the scheduler at configurable PRB utilization levels (30%, 60%, 100%), enabling our power measurement experiments across different load scenarios.
