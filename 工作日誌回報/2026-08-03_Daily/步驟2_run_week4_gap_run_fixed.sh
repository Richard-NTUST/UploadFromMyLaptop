...
# Fixed copy of internship/scripts/run_week4_gap_run.sh
# Fix: this Scaphandre build does not export scaph_host_power_microwatts.
# It reports power per RAPL sub-domain instead (domain="core", "uncore", ...),
# which sum to Package(+DRAM) power == the "Platform Power" defined in
# docs/FinalReport/01_Methodology_Reproducible_Measurement.md section 2.
# Confirmed 2026-08-01 via: curl localhost:8080/metrics
#   (saw scaph_domain_power_microwatts{domain="core"} and domain="uncore",
#    no scaph_host_power_microwatts line at all)
#
# Everything else is unchanged from the original script.
#
# Usage (single laptop, loopback test — see 步驟2_TrackA重現指南.md):
#   TARGET_HOST=localhost OUTPUT_DIR=runs/2026-08-xx/sweep-01 ./步驟2_run_week4_gap_run_fixed.sh
# Run this from inside the internship/ repo root (it calls ./scripts/week3_load_sweep.sh).
...
(
  while true; do
    ...
    curl -s "$SCAPHANDRE_URL" | awk '!/^#/ && $1 ~ /^scaph_domain_power_microwatts(\{|$)/ {sum+=$NF} END {print sum+0}'
